...
func DumpRepository(url string, port int, repo string, auth client.AuthConfig, outputDir string, cli *client.Client) error {
	...
	if manifest.Config.Digest != "" {
		...
		blobFile := filepath.Join(outputDir, repo, fmt.Sprintf("config_%s%s", manifest.Config.Digest, ext))
	...
	for _, layer := range manifest.Layers {
		if layer.Digest != "" {
			...
			blobFile := filepath.Join(outputDir, repo, fmt.Sprintf("layer_%s%s", layer.Digest, ext))
